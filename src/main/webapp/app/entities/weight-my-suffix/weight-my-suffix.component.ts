...
@Component({
    selector: 'jhi-weight-my-suffix',
    templateUrl: './weight-my-suffix.component.html'
})
export class WeightMySuffixComponent implements OnInit, OnDestroy {

currentAccount: any;
    weights: WeightMySuffix[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/weight-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/weight-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: WeightMySuffix) {
        return item.id;
    }
