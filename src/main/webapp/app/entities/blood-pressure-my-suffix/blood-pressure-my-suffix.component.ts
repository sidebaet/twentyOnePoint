...
@Component({
    selector: 'jhi-blood-pressure-my-suffix',
    templateUrl: './blood-pressure-my-suffix.component.html'
})
export class BloodPressureMySuffixComponent implements OnInit, OnDestroy {

currentAccount: any;
    bloodPressures: BloodPressureMySuffix[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/blood-pressure-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/blood-pressure-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: BloodPressureMySuffix) {
        return item.id;
    }
